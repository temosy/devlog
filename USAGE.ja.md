...
Claude Code と Codex のセッション記録、および git ログから日報を自動生成する CLI ツールです。
...
1. コーディングエージェントのセッション記録 — Claude Code（`~/.claude/projects/**/*.jsonl`）と Codex（`~/.codex/sessions/**/*.jsonl`）— から、その日のセッションタイトル・ユーザーの依頼文・編集ファイルを抽出します。tool の実行結果・skill の注入文・AGENTS.md/環境情報の注入などのノイズは自動で除外されます
...
各セッションには出典タグ（`[Claude Code]` / `[Codex]`）が付き、同じリポジトリでの両エージェントの作業は 1 つのプロジェクト見出しにまとめられます。
...
リポジトリの自動発見は「その日エージェントで触ったリポジトリ」しか見つけられません。手作業だけで進めた日のコミットも拾いたいリポジトリは `repos` に列挙しておいてください。いずれかのエージェントのディレクトリが存在しない場合、そのソースは単にスキップされます。
...
- Codex のセッションにはタイトルが無いため最初のプロンプトを代用し、セッション単位の git ブランチ情報もありません。Codex のシェル操作は不透明な JS として実行されるため省略しますが、編集ファイルは取得しています
